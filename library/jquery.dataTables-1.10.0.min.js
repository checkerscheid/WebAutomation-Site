...
jQuery.extend( jQuery.fn.dataTableExt.oSort, {
	'file_size-asc' : function(a, b) {
		var x = parseFloat(a); if (isNaN(x)) { x = -1; }
		var y = parseFloat(b); if (isNaN(y)) { y = -1; }
		a = a.replace(/\s+?\/.*/,'');
		b = b.replace(/\s+?\/.*/,'');
		var x_unit = a.match(/GB/i) ? 1024 * 1024 * 1024
	               : a.match(/MB/i) ? 1024 * 1024
	               : a.match(/KB/i) ? 1024
	               : 1;
		var y_unit = b.match(/GB/i) ? 1024 * 1024 * 1024
	...
	'file_size-desc': function(a, b) {
		return jQuery.fn.dataTableExt.oSort['file_size-asc'](b,a);
	}
});
...
function getXY(TheDate) {
	var x = 0;
	var deDate, deTime, deDate2;
	var regex = /[0-9]{1,2}\.[0-9]{1,2}\.[0-9]{2,4} ([0-2][0-9]|[0-9]):[0-5][0-9]:[0-9][0-9]/g;
	if(regex.test(TheDate)) {
		deDate = $.trim(TheDate).split(' ');
		deTime = deDate[1].split(':');
		deDate2 = deDate[0].split('.');
		x = ((deDate2[2] * 10000000000) + (deDate2[1] * 100000000) + (deDate2[0] * 1000000) + (deTime[0] * 10000) + (deTime[1] * 100) + deTime[2]) * 1;
	}
	regex = /[0-9]{1,2}\.[0-9]{1,2}\.[0-9]{2,4} ([0-2][0-9]|[0-9]):[0-5][0-9]/g;
	if(regex.test(TheDate) && x == 0) {
		deDate = $.trim(TheDate).split(' ');
		deTime = deDate[1].split(':');
		deDate2 = deDate[0].split('.');
		x = ((deDate2[2] * 10000000000) + (deDate2[1] * 100000000) + (deDate2[0] * 1000000) + (deTime[0] * 10000) + (deTime[1] * 100)) * 1;
	}
	regex = /[0-9]{1,2}\.[0-9]{1,2}\.[0-9]{2,4}/g;
	if(regex.test(TheDate) && x == 0) {
		deDate = $.trim(TheDate).split('.');
		x = ((deDate2[2] * 10000000000) + (deDate2[1] * 100000000) + (deDate2[0] * 1000000)) * 1;
	}
	if(x == 0) {
		x = -Infinity;
	}
