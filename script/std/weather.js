...
p.page.load = function() {
	...
	$.get('std.weather.forecastLines.req', function(data) {
		...
		var markings = [
			...
			{ color: "#555", lineWidth: 1, xaxis: { from: marker6.ts, to: marker6.ts } },
			{ color: "rgba(85,85,85,0.2)", linewidth: 1, y2axis: { from: 0, to: 100 } }
		];
		var dataset = [
			{ label: "Sonne", data: data.Cloud, lines: { show: true, fill: 0.1 }, color: "rgba(200,140,30,0.7)", yaxis: 2 },
		...
		var o6 = plot.pointOffset({ x: marker6.ts, y: 0});
		...
		$('#forecastlinesplaceholder').append(
			"<div style='position:absolute;left:" + (o6.left + 10) + "px;top:10px;color:#666;'>" + marker6.d + "</div>");
